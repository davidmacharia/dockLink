...
public class PlanningFilesPanel extends JPanel implements Dashboard.Refreshable {
    ...
    private void loadSelectedPlanDetails() {
        ...
        if (selectedRow != -1) {
            int planId = (int) allPlansTablePanel.getPlansTable().getValueAt(selectedRow, 1); // ID is at index 1
    ...
    private void sendBillingToReception() {
        ...
        if (existingBilling != null) {
            ...
            // For now, let's assume if it's a returned plan, we can create a new billing or update.
